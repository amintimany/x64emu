...
let make_heap_cell_display doc address machine memory_table =
  ...
  let make_address (n : int) =
    Machine.int64_to_hex_string (Int64.of_int n)
  in
  let make_value (n : int) =
    Machine.int_byte_to_hex n
  in
...
let make_machine_display doc machine x64emu_register_table x64emu_program_table x64emu_flags_table x64emu_heap_table x64emu_stack_table =
  ...
  let stack_update = make_stack_display_for_machine doc (ref (Int64.to_int !(machine.Machine.rsp))) machine x64emu_stack_table in
